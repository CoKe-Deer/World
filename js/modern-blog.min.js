...
var demo = (function(window, undefined) {
    ...
    var SELECTORS = {pattern: '.pattern',card: '.card',cardImage: '.card__image',cardClose: '.card__btn-close',};
    var CLASSES = {patternHidden: 'pattern--hidden',polygon: 'polygon',polygonHidden: 'polygon--hidden'};
    var polygonMap = {paths: null,points: null};
    var layout = {};
    function init() {
        var pattern = Trianglify({width: window.innerWidth,height: window.innerHeight,cell_size: 90,variance: 1,stroke_width: 1,x_colors: 'random',y_colors: 'random'}).svg();
    ...
    function _playSequence(isOpenClick, id, e) {
        ...
        var ioa = document.getElementById("xyy");
        if (!card.isOpen) {
            sequence.add(tweenOtherCards);
            sequence.add(card.openCard(_onCardMove), 0);
            
            ioa.style.display="none";
            //iiiiiiiiiii.style.add("display","none");
        } else {
            ...
            ioa.style.display='inline';
            //iiiiiiiiiii.hide();
        }
        sequence.play();
        setTimeout('_setPatternBgImg(e.target);',5000);
    ...
    function _onCardMove(track) {
        ...
        var center = {x: track.x,y: track.y};
